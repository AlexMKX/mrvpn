...
Expiration happens on calling expire, object is locked during cleanup from expired
values.

>>> ManualExpiringDict(max_age_seconds=10)
...
class ManualExpiringDict(OrderedDict):
    def __init__(self, max_age_seconds, items=None):
        ...
        if not self.__is_instance_of_expiring_dict(items):
            self.__assertions( max_age_seconds)
        ...
        if items is not None:
            if self.__is_instance_of_expiring_dict(items):
                self.__copy_expiring_dict(max_age_seconds, items)
    ...
    def __reduce__(self):
        reduced = self.__class__, (self.max_age, ('reduce_result', self.items_with_timestamp()))
        return reduced

    def __assertions(self, max_age_seconds):
    ...
    def __copy_expiring_dict(self, max_age_seconds, items):
    ...
    def expire(self):
        t = time.time()
        candidates = [x[0] for x in OrderedDict.items(self) if x[1][1] + self.max_age < time.time()]
        for c in candidates:
            del self[c]
